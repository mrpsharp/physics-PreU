...
\fbox{\begin{minipage}[t]{\textwidth}
	...
	A ball is thrown with a velocity of \SI{5}{m.s^-1} from a height of \SI{1.2}{m}. If its initial angle to the horizonal is \SI{50}{degrees} calclate the distance it travels before it hits the ground.
	...
	The time for the ball to reach the ground can now be calculated using the vertical motion and the equation $s=ut+\frac{1}{2}at^2$, setting $s = \SI{-1.2}{m}$. This gives $t=$.

	Finally, the horizontal distance is calculated using the simple constant velocity formula to give $x=$.
\end{minipage}}
